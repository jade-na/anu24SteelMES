...
namespace grpcDummyMesServer
{
	...
	{
		private UserSessionManager m_UserSessionManager = new UserSessionManager();
		...
		public override Task<LogoutReply> Logout(LogoutRequest request, ServerCallContext context)
		{
			...
			{
				// 로그아웃 처리: 세션 삭제
				bool success = m_UserSessionManager.RemoveUserSession(request.UserId);
				if (success)
				{
					result.Success = true;
		...
		// 강제 로그아웃 gRPC 메서드
		public override async Task<ForceLogoutReply> ForceLogout(LogoutRequest request, ServerCallContext context)
		{
			...
			{
				// 이미 로그인된 사용자가 있는지 확인
				forceExit = m_UserSessionManager.IsUserLoggedIn(request.UserId);

				if (forceExit)
				{
					// 중복 로그인 발견 시
					result.Message = "이미 로그인된 세션이 있습니다. 기존 세션을 종료하시겠습니까?";
					result.Success = false;  // 세션 강제 종료 실패
					result.PromptUser = true;  // 클라이언트에게 메시지를 띄우도록 알림
				}
				else
				{
					// 기존 세션이 없으면 바로 로그인
					if (m_UserSessionManager.AddUserSession(request.UserId, out forceExit))
					{
						result.Message = "새로운 세션으로 로그인되었습니다.";
						result.Success = true;
						result.PromptUser = false;  // 메시지 박스는 띄우지 않음
					}
					else
					{
						result.Message = "세션 추가 실패.";
						result.Success = false;
						result.PromptUser = false;
					}
				}
			}
			catch (Exception ex)
